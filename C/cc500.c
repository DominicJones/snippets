 ...
 *
 * gcc -m32 -o cc500 cc500.c
 * ./cc500 < program.c > program
 * chmod +x program
 * ./program
 */
...
void get_token()
{
  ...
  {
    w = 0;
    while ((next_char == ' ') | (next_char == 9) | (next_char == 10))
      next_char = getchar();

    i = 0;
    while ((('a' <= next_char) & (next_char <= 'z')) |
	   (('0' <= next_char) & (next_char <= '9')) | (next_char == '_'))
      takechar();
...
void sym_get_value(char *s)
{
  ...
  save_int(code + code_pos - 4, load_int(table + t + 2));
  ...
  {
    save_int(table + t + 2, code_pos + code_offset - 4);
  }
  /* local variable */
  else if (table[t + 1] == 'L')
  {
...
void be_start()
{
  ...
  save_int(code + 85, code_pos - 89); /* entry set to first thing in file */
}


void be_finish()
{
  save_int(code + 68, code_pos);
  save_int(code + 72, code_pos);
...
int primary_expr()
{
  ...
  {
    emit(5, "\xb8...."); /* mov $x,%eax */
    save_int(code + code_pos - 4, token[1]);
    type = 3;
  }
  else if (token[0] == '"')
  {
...
int postfix_expr()
{
  ...
  else if (accept("("))
  {
    ...
    save_int(code + code_pos - 4, (stack_pos - s - 1) << 2);
...
void statement()
{
  ...
  else if (accept("if"))
  {
    ...
    emit(5, "\xe9...."); /* jmp ... */
    p2 = code_pos;
    save_int(code + p1 - 4, code_pos - p1);
    if (accept("else"))
      statement();
    save_int(code + p2 - 4, code_pos - p2);
  }
  else if (accept("while"))
  {
    ...
    statement();
    emit(5, "\xe9...."); /* jmp ... */
    save_int(code + code_pos - 4, p1 - code_pos);
    save_int(code + p2 - 4, code_pos - p2);
  }
